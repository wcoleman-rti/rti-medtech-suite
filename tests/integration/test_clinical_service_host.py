...
def clinical_service_host():
    ...
    assert ready, "clinical-service-host did not publish HostCatalog within 10 s"
    yield proc
...
class TestClinicalHostCatalog:
    """Verify HostCatalog publication on startup."""

    def test_host_catalog_published(self, clinical_service_host, catalog_reader):
        """Clinical Service Host publishes HostCatalog with correct services."""
        ...
        assert "BedsideMonitorService" in catalog.supported_services
        assert "DeviceTelemetryService" in catalog.supported_services
...
class TestClinicalRpcControl:
    ...
    def test_get_capabilities(self, clinical_service_host, rpc_requester):
        ...
        assert "BedsideMonitorService" in result.supported_services
        assert "DeviceTelemetryService" in result.supported_services
    ...
    def test_start_bedside_monitor(
        ...
        _wait_for_replier(rpc_requester, timeout_sec=10)
        call = _make_start_call("BedsideMonitorService")
        reply = _send_rpc(rpc_requester, call)
        ...
        while time.time() < deadline:
            ...
            for s in samples:
                if (
                    ...
                    and s.data.service_id == "BedsideMonitorService"
            ...
            if found_running:
                break
            time.sleep(0.2)
        assert found_running, "BedsideMonitorService never reached RUNNING"
    ...
    def test_start_device_telemetry(
        ...
        _wait_for_replier(rpc_requester, timeout_sec=10)
        call = _make_start_call("DeviceTelemetryService")
        reply = _send_rpc(rpc_requester, call)
        assert reply is not None
        result = reply.start_service.result.return_
        assert result.code == Orchestration.OperationResultCode.OK
        ...
        while time.time() < deadline:
            ...
            for s in samples:
                if (
                    s.info.valid
                    and s.data.host_id == HOST_ID
                    and s.data.service_id == "DeviceTelemetryService"
                    and s.data.state == Orchestration.ServiceState.RUNNING
                ):
                    found_running = True
                    break
            if found_running:
                break
            time.sleep(0.2)
        assert found_running, "DeviceTelemetryService never reached RUNNING"
    ...
    def test_stop_bedside_monitor(
        ...
        _wait_for_replier(rpc_requester, timeout_sec=10)
        call = _make_stop_call("BedsideMonitorService")
        reply = _send_rpc(rpc_requester, call)
        assert reply is not None
        ...
        while time.time() < deadline:
            ...
            if found_stopped:
                break
            time.sleep(0.2)
        assert found_stopped, "BedsideMonitorService never reached STOPPED"
    ...
    def test_stop_device_telemetry(
        ...
        _wait_for_replier(rpc_requester, timeout_sec=10)
        call = _make_stop_call("DeviceTelemetryService")
        reply = _send_rpc(rpc_requester, call)
        assert reply is not None
        result = reply.stop_service.result.return_
        assert result.code == Orchestration.OperationResultCode.OK
        ...
        while time.time() < deadline:
            ...
            if found_stopped:
                break
            time.sleep(0.2)
        assert found_stopped, "DeviceTelemetryService never reached STOPPED"
