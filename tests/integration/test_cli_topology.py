...
class TestTopologyBasic:
    ...
    def test_topology_renders_tree(self, mock_subprocess) -> None:
        ...
        network_names = "medtech_surgical-net\nmedtech_hospital-net\n"

        surgical_inspect = _make_network_inspect(
            "medtech_surgical-net",
            "172.18.0.0/16",
            {
                "c1": _container_entry("cloud-discovery-service", "172.18.0.2"),
                "c2": _container_entry("clinical-service-host-or1", "172.18.0.3"),
            },
        )
        hospital_inspect = _make_network_inspect(
            ...
            "172.19.0.0/16",
            {
                "c3": _container_entry("medtech-gui", "172.19.0.2"),
            },
        )
        ...
        def subprocess_side_effect(args, **kwargs):
            ...
            if "network" in args and "ls" in args:
                ...
                net_name = args[-1]
                if net_name == "medtech_surgical-net":
                    result.stdout = surgical_inspect
                elif net_name == "medtech_hospital-net":
        ...
        assert "medtech_surgical-net" in result.output
        assert "medtech_hospital-net" in result.output
        # Should contain container names
        assert "cloud-discovery-service" in result.output
        assert "clinical-service-host-or1" in result.output
...
class TestTopologyMultiHospital:
    ...
    def test_groups_by_hospital(self, mock_subprocess) -> None:
        ...
        network_names = (
            ...
            "medtech_hospital-b_surgical-net\n"
            "medtech_wan-net\n"
        )
        ...
        def subprocess_side_effect(args, **kwargs):
            ...
            if "network" in args and "ls" in args:
                ...
                net_name = args[-1]
                data = {
                    "medtech_hospital-a_surgical-net": a_inspect,
                    "medtech_hospital-b_surgical-net": b_inspect,
...
class TestDockgraphLabel:
    ...
    def test_dockgraph_has_dynamic_label(
        ...
        dockgraph_calls = [
            c
            for c in mock_launch_run.call_args_list
