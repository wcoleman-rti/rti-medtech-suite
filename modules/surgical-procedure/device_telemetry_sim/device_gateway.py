...
class DeviceGateway:
    ...
    Exclusive ownership for primary/backup failover is configured at the
    QoS profile level (XML). The DeviceTelemetry topic uses State pattern
    QoS; for exclusive ownership deployments, an alternative XML profile
    with EXCLUSIVE ownership_kind and configurable strength is used. This
    is tested independently via conftest-based DDS fixtures in
    ...
    def __init__(
        ...
        # --- Profile ---
        profile_name = os.environ.get("MEDTECH_SIM_PROFILE", "stable")
        self._profile: DeviceProfile = DEVICE_PROFILES.get(
            profile_name, DEVICE_PROFILES["stable"]
        )
        ...
        self._heartbeat_interval = float(hb_env) if hb_env else 0.0
