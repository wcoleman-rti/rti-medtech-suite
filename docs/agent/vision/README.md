...
5. **DomainParticipant partitions drive static context isolation.** Context-based isolation (room, procedure, tier, etc.) is achieved via **DomainParticipant** partitions set once at participant startup — not via separate domain IDs, domain tags, topic names, or Publisher/Subscriber partition QoS. Publisher/Subscriber Partition QoS is **never used** in this project. The same application binary runs in any room; the DomainParticipant partition is assigned at startup from context (constructor parameters or startup configuration objects) and **never changed at runtime**.
...
7. **All QoS is set in XML configuration, never in code.** This is a strict requirement. All DDS QoS, transport settings, and discovery peer configuration come from XML profiles. No QoS is constructed or modified programmatically. Modules reference named QoS profiles via `QosProvider` — they never call QoS setter APIs. **Two approved programmatic exceptions:** (1) the factory-level XTypes compliance mask (bit `0x00000020`, `accept_unknown_enum_value`) must be set programmatically before any DomainParticipant is created, as it has no XML equivalent; (2) **DomainParticipant partition** (`DomainParticipantQos.partition.name`) is set programmatically immediately after `create_participant_from_config()` because it is context-dependent startup configuration that the application already holds — no XML substitution variable or environment variable is used. See [data-model.md — Pre-Participant Initialization](data-model.md) and [dds-consistency.md — Initialization Sequence](dds-consistency.md). Those sections also document type registration requirements.
8. **Configuration over code.** Beyond QoS, all runtime-variable DDS settings (domain IDs, domain tags, transport, discovery peers) come from XML configuration and environment parameters — not hardcoded in application logic. **Exception:** DomainParticipant partition is set in code after participant creation — see Principle 7.
